...
split_poly <- function(sf_poly, n_areas, seed = floor(runif(1, 1000,9999))){
  ...
  points <- do.call(rbind, sf::st_geometry(points_rnd)) %>%
    dplyr::as_tibble() %>% stats::setNames(c("lon","lat"))
  k_means <- tryCatch(
  ...
  points_sf <- sf::st_as_sf(as.data.frame(k_means$centers), coords = c("lon", "lat")) |>
    sf::st_set_crs(sf::st_crs(sf_poly))
  # equal_areas <- sf::st_intersection(voronoi_sf, sf_poly)
  # equal_areas$area <- sf::st_area(equal_areas)
  equal_areas <- sf::st_union(points_sf) |>
    sf::st_voronoi() |>
    sf::st_collection_extract() |>
    sf::st_intersection(sf_poly) |>
    sf::st_as_sf()
  return(equal_areas)
}
