...
mapadminSRV <- function(id, boundaryfile = reactive({
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      assingTabProxy <- DT::dataTableProxy(
        "assignTable",
      ...
      #   MAP ASSIGNMENT & LOG
      toassingoverview <- reactiveVal(data.table::data.table(Team = character(0), Maps = character(0)))
      observeEvent(input$suso_assignmap,
        {
          ...
          # Correct possible side effect from data.table::data.table
          ...
          for(i in 1:nrow(mapsvsel)){
            SurveySolutionsAPI::suso_mapassign(fileName = mapsvsel$Maps[i], 
                                               userName = mapsvsel$Team[i], 
                                               workspace = input$susoWS)
          }
      ...
      output$assignoverviewTable <- DT::renderDataTable({
        ...
        shiny::validate(need(apiCheck() == 200, message = F))
        # tab<-data.table::data.table(Team=character(0), Maps=character(0))
