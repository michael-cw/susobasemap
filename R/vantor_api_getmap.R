...
get_maxar_wms_basemap <- function(bbox,
  ...
  if (is.null(width) || is.null(height)) {
    ...
    if (verbose) {
      cat(sprintf("Calculated image dimensions: %d x %d pixels\n", width, height))
      cat(sprintf("For %.2f m resolution over %.1f x %.1f meters\n", 
    ...
    if (width > max_dim || height > max_dim) {
      ...
      if (verbose) {
        cat(sprintf("Dimensions limited to: %d x %d pixels (max: %d)\n", 
  ...
  if (verbose) {
    cat("Requesting WMS basemap from Maxar...\n")
    cat(sprintf("Product: %s\n", product))
    cat(sprintf("Bounding box (EPSG:3857): %.2f, %.2f, %.2f, %.2f\n", 
  ...
  tryCatch({
    ...
    if (resp_status(response) != 200) {
      ...
      error_msg <- tryCatch(
        ...
        error = function(e) resp_status_desc(response)
      )
      stop(paste("WMS request failed with status:", 
    ...
    if (!grepl("image/(geo)?tiff", content_type, ignore.case = TRUE)) {
      ...
      error_body <- resp_body_string(response)
    ...
    if (verbose) {
      ...
      cat(sprintf("GeoTIFF saved successfully: %s (%.2f MB)\n", 
    ...
    stop(paste("Error retrieving WMS basemap:", e$message))
  })
}
...
convert_bbox_to_3857 <- function(bbox_4326) {
  ...
  bbox_sf <- st_bbox(c(xmin = bbox_4326[1], 
                       ymin = bbox_4326[2],
                       xmax = bbox_4326[3], 
                       ymax = bbox_4326[4]),
                     crs = st_crs(4326))
