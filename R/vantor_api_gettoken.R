...
get_maxar_token <- function(username, password, verbose = FALSE) {
  ...
  tryCatch({
    ...
    if (resp_status(response) != 200) {
      stop(paste("Authentication failed with status:", 
                 resp_status(response),
                 "\nReason:", resp_status_desc(response)))
    }
    ...
    cat("Successfully retrieved token.\n")
    cat(paste("Token expires in:", token_data$expires_in, "seconds\n"))
    ...
    stop(paste("Error retrieving token:", e$message))
  })
}
...
refresh_maxar_token <- function(refresh_token, verbose = FALSE) {
  ...
  tryCatch({
    ...
    # Print success message
    cat("Successfully refreshed token.\n")
    cat(paste("Token expires in:", token_data$expires_in, "seconds\n"))
